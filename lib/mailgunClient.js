...
/**
 * Inboxes for contact-form notifications.
 * Prefer @swimcare.be: Microsoft 365 on soloswim.be has been blocking some Mailgun IPs (550 5.7.511).
 */
export function getMailgunContactTo() {
  const raw = env("MAILGUN_CONTACT_TO");
  const value = raw || "kristof@swimcare.be";
  return value
    .split(",")
    .map((part) => part.trim())
    .filter(Boolean);
