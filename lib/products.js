...
export function getSwimmingEquipmentData() {
  ...
  // Training gear only (exclude swim care / verzorging)
  return allProducts.filter(
    (product) => product.isAccessoire === true && product.isVerzorging !== true
  );
}

export function getZwemverzorgingData() {
  const allProducts = getSortedProductsData();
  return allProducts.filter((product) => product.isVerzorging === true);
}

export function getSwimmingSchemasData() {
  const allProducts = getSortedProductsData();
  // Filter out accessories and care products
  return allProducts.filter(
    (product) => product.isAccessoire !== true && product.isVerzorging !== true
  );
}
