...
function FullWidthImageCard({ img, number, title, text, alignRight }) {
  return (
    <div className={`flex flex-col sm:flex-row max-w-4xl lg:max-w-9/10 ${alignRight && "ml-auto"}`}>
      ...
      <div className={`bg-white shadow-custom3 rounded-b-xl sm:rounded-b-none ${alignRight ? "sm:rounded-l-xl sm:ml-5" : "sm:rounded-r-xl sm:mr-5"} p-5 sm:px-7 md:px-10 lg:p-11 xl:px-16 xl:py-14 flex flex-col gap-2 sm:gap-3 md:gap-4 lg:gap-8`}>
        <div className="flex flex-row gap-5 items-center">
          <div>
            <p className="text-6xl lg:text-7xl text-navy-light1 font-lexend">
              {number}
            </p>
          </div>
          <div>
